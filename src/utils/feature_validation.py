...
def validate_features(X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
    ...
    validation_results = {"is_valid": True, "errors": [], "warnings": [], "stats": {}}
    ...
    if missing_features.any():
        validation_results["errors"].append(
            f"Missing values in features: {missing_features[missing_features > 0]}"
        )
        validation_results["is_valid"] = False
    ...
    if inf_features.any():
        validation_results["errors"].append(
            f"Infinite values in features: {inf_features[inf_features > 0]}"
        )
        validation_results["is_valid"] = False
