...
class FeatureEngineer:
    ...
    def create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if df.empty or len(df) < self.lookback_periods:
            raise ValueError(f"Insufficient data: need at least {self.lookback_periods} periods")
        ...
        logger.info(f"Created {len(features_df.columns)} features from {len(df)} data points")
        return features_df
