...
class AlertingEngine:
    ...
    def evaluate_prediction(self, prediction_result: Dict[str, Any]) -> List[Alert]:
        ...
        alerts.extend(self._check_price_alerts(
    ...
    def _check_price_alerts(self, symbol: str, current_price: float, 
                           predicted_price: float, price_change_pct: float) -> List[Alert]:
        """Check for price-based alerts"""
        alerts = []
        
        # Price increase alert
        increase_rule = self.alert_rules.get(AlertType.PRICE_INCREASE)
        if (increase_rule and increase_rule.enabled and 
            price_change_pct >= increase_rule.threshold_pct):
        ...
        if (decrease_rule and decrease_rule.enabled and 
            price_change_pct <= decrease_rule.threshold_pct):
    ...
    def get_recent_alerts(self, limit: int = 10) -> List[Alert]:
        ...
        return sorted(self.alert_history, 
